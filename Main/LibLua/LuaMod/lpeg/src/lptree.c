...
// [:COMET:]:160123: add always this header to Lua module files
#include "lpdef.h"
//
...
#define LUAEXT_VERSION_STRING "\nLua Library for PEG v1.0 (C) Lua.org & PUC-Rio\n"
...
static void fixonecall(lua_State *L, int postable, TTree *g, TTree *t)
{
    ...
    if (n == 0) {                   /* no position? */
        ...
        luaL_errorlevel(L, "rule '%s' undefined in given grammar", val2str(L, -1));
    }
    t->tag = TCall;
    ...
    switch (t->tag) {
        ...
        case TOpenCall:
        {
            if (g != NULL) /* inside a grammar? */
                ...
                luaL_errorlevel(L, "rule '%s' used outside a grammar", val2str(L, -1));
            }
            break;
        }
        case TSeq:
...
static int addtoktable(lua_State *L, int idx)
{
    if (lua_isnil(L, idx)) /* nil value? */
        ...
        if (n >= USHRT_MAX)
            luaL_errorlevel(L, "too many Lua values in pattern");
...
static int lp_star(lua_State *L)
{
    ...
    if (n >= 0) { /* seq tree1 (seq tree1 ... (seq tree1 (rep tree1))) */
        ...
        if (nullable(tree1))
            luaL_errorlevel(L, "loop body may accept empty string");
...
static void getfirstrule(lua_State *L, int arg, int postab)
{
    ...
    if (!testpattern(L, -1)) { /* initial rule not a pattern? */
        if (lua_isnil(L, -1))
            luaL_errorlevel(L, "grammar has no initial rule");
        else
            luaL_errorlevel(L, "initial rule '%s' is not a pattern", lua_tostring(L, -2));
    }
...
static int collectrules(lua_State *L, int arg, int *totalsize)
{
    ...
    while (lua_next(L, arg) != 0) {
        ...
        if (!testpattern(L, -1)) /* value is not a pattern? */
            luaL_errorlevel(L, "rule '%s' is not a pattern", val2str(L, -2));
    ...
    for (i = npassed - 1; i >= 0; i--) { /* search for a repetition */
        for (j = i - 1; j >= 0; j--) {
            if (passed[i] == passed[j]) {
                ...
                return luaL_errorlevel(L, "rule '%s' may be left recursive", val2str(L, -1));
            }
        }
    }
    return luaL_errorlevel(L, "too many left calls in grammar");
}
...
static void verifygrammar(lua_State *L, TTree *grammar)
{
    ...
    for (rule = sib1(grammar); rule->tag == TRule; rule = sib2(rule)) {
        ...
        if (checkloops(sib1(rule))) {
            ...
            luaL_errorlevel(L, "empty loop in rule '%s'", val2str(L, -1));
        }
    }
    assert(rule->tag == TTrue);
}
...
static const struct luaL_Reg pattreg[] = {
    { "ptree", lp_printtree },
    { "pcode", lp_printcode },
    { "match", lp_match },
    { "B", lp_behind },
...
static const struct luaL_Reg metareg[] = {
    { "__mul", lp_seq },
    { "__add", lp_choice },
    { "__pow", lp_star },
    { "__gc", lp_gc },
...
LIBLPEG_API int luaopen_lpeg(lua_State *L)
{
