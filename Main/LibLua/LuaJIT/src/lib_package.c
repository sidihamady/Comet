...
static int lj_cf_package_require(lua_State *L)
{
  ...
  char *pszM = (char *) strstr((const char *) szModuleName, ".lua");
  if (NULL == pszM) {
      pszM = (char *) strstr((const char *) szModuleName, ".luac");
  }
  if (NULL == pszM) {
      pszM = (char *) strstr((const char *) szModuleName, ".dll");
  }
  if (NULL == pszM) {
      pszM = (char *) strstr((const char *) szModuleName, ".so");
  }
  if (NULL != pszM) {
      *pszM = '\0';
  }
  ...
  lua_getfield(L, 2, (const char *) szModuleName);
  if (lua_toboolean(L, -1)) {  /* is it there? */
    if (lua_touserdata(L, -1) == sentinel)  /* check loops */
        luaL_error(L, "loop or previous error loading module " LUA_QS, (const char *) szModuleName);
  ...
  for (i = 1; ; i++) {
    ...
    if (lua_isnil(L, -1))
        luaL_error(L, "module " LUA_QS " not found:%s", (const char *) szModuleName, lua_tostring(L, -2));
    lua_pushstring(L, (const char *) szModuleName);
  ...
  lua_setfield(L, 2, (const char *) szModuleName);  /* _LOADED[name] = sentinel */
  lua_pushstring(L, (const char *) szModuleName);  /* pass name as argument to module */
  lua_call(L, 1, 1);  /* run loaded module */
  if (!lua_isnil(L, -1))  /* non-nil return? */
      lua_setfield(L, 2, (const char *) szModuleName);  /* _LOADED[name] = returned value */
  lua_getfield(L, 2, (const char *) szModuleName);
  if (lua_touserdata(L, -1) == sentinel) {   /* module did not set a value? */
    lua_pushboolean(L, 1);  /* use true as result */
    lua_pushvalue(L, -1);  /* extra copy to be returned */
    lua_setfield(L, 2, (const char *) szModuleName);  /* _LOADED[name] = true */
  }
  lj_lib_checkfpu(L);
