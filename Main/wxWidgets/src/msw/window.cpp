                             ...
                             const wxFont *theFont) const
{
    ...
    SIZE sizeRect;
    TEXTMETRIC tm;
    ::GetTextExtentPoint32(hdc, string, (int) (string.length()), &sizeRect);
...
bool wxWindowMSW::HandleDropFiles(WXWPARAM wParam)
{
    ...
    for ( UINT wIndex = 0; wIndex < gwFilesDropped; wIndex++ )
    {
        ...
        ::DragQueryFile(hFilesInfo, wIndex,
            wxStringBuffer(files[wIndex], len), (UINT) (len));
    }
...
wxWindowMSW::MSWOnMeasureItem(int id, WXMEASUREITEMSTRUCT *itemStruct)
{
    ...
    if ( id == 0 && pMeasureStruct->CtlType == ODT_MENU )
    {
        ...
        pMeasureStruct->itemWidth = (UINT) w;
        pMeasureStruct->itemHeight = (UINT) h;
    ...
    if ( !gs_hasStdCmap )
    {
        ...
        if ( !s_coloursInit )
        {
            ...
            if ( stdColourBitmap.Ok() )
            {
                ...
                {
                    memDC.GetPixel((wxCoord) (i), (wxCoord) (0), &colour);
