...
BEGIN_EVENT_TABLE(wxTextCtrl, wxTextCtrlBase)
    ...
    EVT_DROP_FILES(wxTextCtrl::OnDropFiles)
     ...
     // [:COMET:]:20151031: change the context menu to avoid the language mix
//#if wxUSE_RICHEDIT
            ...
            switch ( ctrl + shift )
            {
                ...
                case 0:
                    if ( IsMultiLine() && vkey == VK_RETURN )
                        return false;
                    if (vkey == VK_DELETE) return false; // [:COMET:]:20150107:
                    // fall through
                case 2:
                    break;
...
void wxTextCtrl::OnKeyDown(wxKeyEvent& event)
{
    ...
    if ( event.GetModifiers() == wxMOD_CONTROL )
    {
         ...
         // [:COMET:]:20151120: enable SelectAll for multiline edit
         else if (HasFlag(wxTE_MULTILINE)) {
             switch (event.GetKeyCode()) {
                 case 'A':
                     SelectAll();
                     return;
...
void wxTextCtrl::OnContextMenu(wxContextMenuEvent& WXUNUSED(event))
{
#if wxUSE_RICHEDIT
    if (IsRich())
    {
