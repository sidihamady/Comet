                          ...
                          const wxString& name)
{
	...
	// [:COMET:]:20151015: statusbar text option added
	m_bDispHelp = false;
	m_strOldStatusText = wxEmptyString;
#endif
...
{
	// [:COMET:]:20151015: restore statusbar text
	// do we have real status text to restore?
	if (m_bDispHelp && (m_statusBarPane >= 0) && (m_strOldStatusText.empty() == false)) {
		wxStatusBar *statbar = GetStatusBar();
		if (statbar) {
			statbar->SetStatusText(m_strOldStatusText, m_statusBarPane);
		}
		m_strOldStatusText.clear();
	}
	//
}
...
{
#if wxUSE_STATUSBAR
	// [:COMET:]:20151015: statusbar text option
	if ((m_statusBarPane < 0) || (m_bDispHelp == false)) {
	...
	if (show) {
		help = text;
		if (m_strOldStatusText.empty()) {
			m_strOldStatusText = statbar->GetStatusText(m_statusBarPane);
			if (m_strOldStatusText.empty()) {
				// use special value to prevent us from doing this the next time
				m_strOldStatusText += _T('\0');
			}
		}
	}
	else {
		// i.e. restore the old one
		help = m_strOldStatusText;
