...
BEGIN_EVENT_TABLE(wxStatusBarBase, wxWindow)
    // [:COMET:]:150815: send double click event to parent (frame)
    EVT_LEFT_DCLICK(wxStatusBarBase::OnDoubleClick)
END_EVENT_TABLE()
...
// [:COMET:]:150815: send double click event to parent (frame)
void wxStatusBarBase::OnDoubleClick(wxMouseEvent& tEvent)
{
    tEvent.Skip();

    wxWindow* pParent = this->GetParent();
    if (pParent) {
        pParent->ProcessEvent(tEvent);
    }
}
