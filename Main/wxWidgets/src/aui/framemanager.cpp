...
{
	unsigned char data[] = { 128, 128, 128, 220, 220, 220, 220, 220, 220, 128, 128, 128 };
...
#else
	int oldLF = dc.GetLogicalFunction();
	dc.SetLogicalFunction(wxXOR);
	dc.DrawRectangle(rect);
	dc.SetLogicalFunction(oldLF);
#endif
...
BEGIN_EVENT_TABLE(wxAuiManager, wxEvtHandler)
    ...
    EVT_LEFT_DCLICK(wxAuiManager::OnDoubleClick)
...
bool wxAuiManager::DoEndResizeAction(wxMouseEvent& event)
{
    ...
    if (m_action_part && m_action_part->type==wxAuiDockUIPart::typeDockSizer)
    {
        ...
        Update();
        Repaint(NULL);
		  wxRect rcT = m_frame->GetClientRect();
		  m_frame->Refresh(true, &rcT);
		  m_frame->Update();
    }
...
void wxAuiManager::OnDoubleClick(wxMouseEvent& event)
{
	event.Skip();
}
...
void wxAuiManager::OnLeftUp(wxMouseEvent& event)
{
    if (m_action == actionResize)
    {
        ...
        if (gs_CurrentDragItem != -1 && wxAuiManager_HasLiveResize(*this))
		  ...
		  event.m_x = m_insize_mouse_move.x;
		  event.m_y = m_insize_mouse_move.y;
...
void wxAuiManager::OnMotion(wxMouseEvent& event)
{
        ...
        if (m_action_part)
        {
            ...
            else
            {
                ...
                wxRect frame_screen_rect = m_frame->GetScreenRect();
					 ...
					 if (m_action_part->orientation == wxVERTICAL) {
						 minSize.x = m_action_part->dock->min_size;
						 if (rect.x < (frame_screen_rect.x + minSize.x)) {
							 rect.x = frame_screen_rect.x + minSize.x;
							 bresized = true;
						 }
						 else if (rect.x >(frame_screen_rect.x + frame_screen_rect.width - minSize.x)) {
							 rect.x = frame_screen_rect.x + frame_screen_rect.width - minSize.x;
							 bresized = true;
						 }
					 }
					 else if (m_action_part->orientation == wxHORIZONTAL) {
						 int iOffTop = 0, iOffBottom = 0;
#if wxUSE_TOOLBAR
						 // if there's a toolbar control, the available
						 // height decreases accordingly
						 if (m_frame && m_frame->IsKindOf(CLASSINFO(wxFrame))) {
							 wxFrame* frame = static_cast<wxFrame*>(m_frame);
							 wxToolBar* toolbar = frame->GetToolBar();
							 if (toolbar && toolbar->IsShown()) {
								 wxSize tool_client_size = toolbar->GetClientSize();
								 iOffTop = tool_client_size.GetHeight();
							 }
						 }
#endif
#if wxUSE_STATUSBAR
						 ...
						 if (m_frame && m_frame->IsKindOf(CLASSINFO(wxFrame))) {
							 ...
							 if (status && status->IsShown()) {
								 wxSize status_client_size = status->GetClientSize();
								 iOffBottom = status_client_size.GetHeight();
							 }
						 }
#endif
						 minSize.y = m_action_part->dock->min_size;
						 if (rect.y < (frame_screen_rect.y + iOffTop + minSize.y)) {
							 rect.y = frame_screen_rect.y + iOffTop + minSize.y;
							 bresized = true;
						 }
						 else if (rect.y >(frame_screen_rect.y + frame_screen_rect.height - minSize.y - iOffBottom)) {
							 rect.y = frame_screen_rect.y + frame_screen_rect.height - minSize.y - iOffBottom;
							 bresized = true;
						 }
					 }
					 if (false == bresized) {
						 m_insize_mouse_move = mouse_pos;
					 }
					 else {
						 m_insize_mouse_move = wxPoint(rect.x, rect.y);
						 m_insize_mouse_move = m_frame->ScreenToClient(m_insize_mouse_move);
					 }
                // draw new resize hint, if it's inside the managed frame
					 if (!frame_screen_rect.Contains(rect))
					 {
						 if (rect.x < frame_screen_rect.x)
							 rect.x = frame_screen_rect.x;
						 if (rect.y < frame_screen_rect.y)
							 rect.y = frame_screen_rect.y;
						 if ((rect.x + rect.width) > (frame_screen_rect.x + frame_screen_rect.width))
							 rect.width -= (rect.x + rect.width) - (frame_screen_rect.x + frame_screen_rect.width);
						 if ((rect.y + rect.height) > (frame_screen_rect.y + frame_screen_rect.height))
							 rect.width -= (rect.y + rect.height) - (frame_screen_rect.y + frame_screen_rect.height);
					 }
                if (frame_screen_rect.Contains(rect))
                {
    ...
    {
        int drag_x_threshold = wxSystemSettings::GetMetric(wxSYS_DRAG_X) * 2;
		  if (drag_x_threshold < 32) {
			  drag_x_threshold = 32;
		  }
		  int drag_y_threshold = wxSystemSettings::GetMetric(wxSYS_DRAG_Y) * 2;
		  if (drag_y_threshold < 32) {
			  drag_y_threshold = 32;
		  }
