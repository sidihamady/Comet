...
DEFINE_EVENT_TYPE(wxEVT_COMMAND_AUINOTEBOOK_FILELIST)
...
DEFINE_EVENT_TYPE(wxEVT_COMMAND_AUINOTEBOOK_MAINFILELIST)
...
wxAuiDefaultTabArt::wxAuiDefaultTabArt()
{
    ...
    // >> [:COMET:]:130525: Change the folowing colors from *wxRED to wxColour(192,0,0) and *wxBLACK to wxColour(50,100,150)

    m_active_close_bmp = wxAuiBitmapFromBits(close_bits, 16, 16, wxColour(200,100,80));
    m_disabled_close_bmp = wxAuiBitmapFromBits(close_bits, 16, 16, wxColour(150,170,190));

    m_active_left_bmp = wxAuiBitmapFromBits(left_bits, 16, 16, wxColour(50,100,150));
    m_disabled_left_bmp = wxAuiBitmapFromBits(left_bits, 16, 16, wxColour(150,170,190));

    m_active_right_bmp = wxAuiBitmapFromBits(right_bits, 16, 16, wxColour(50,100,150));
    m_disabled_right_bmp = wxAuiBitmapFromBits(right_bits, 16, 16, wxColour(150,170,190));

    m_active_windowlist_bmp = wxAuiBitmapFromBits(list_bits, 16, 16, wxColour(50,100,150));
    m_disabled_windowlist_bmp = wxAuiBitmapFromBits(list_bits, 16, 16, wxColour(150,170,190));
...
{
    // draw background
   wxColor top_color       = wxAuiStepColour(m_base_colour, 120);
   wxColor bottom_color   = wxAuiStepColour(m_base_colour, 170);
   wxRect r;

   if (m_flags &wxAUI_NB_BOTTOM)
       r = wxRect(rect.x, rect.y, rect.width, rect.height);
...
*/
    ...
    wxString draw_text = wxAuiChopText(dc,
     ...
     // [:COMET:]:150620:MOD: color for modified state
     bool bMod = caption.EndsWith(wxT("*"));
     wxColour clrT = dc.GetTextForeground();
     if (bMod) {
         dc.SetTextForeground(wxColour(180, 110, 5));
     }
     //
                                     ...
                                     int active_idx)
{
    ...
    for (i = 0; i < count; ++i)
    {
        ...
        if (caption.IsEmpty())
          ...
          // [:COMET:]:150620:MOD: color and font change for modified/selected states

          iWc = wxSystemSettings::GetMetric(wxSYS_MENU_Y);
          if (iWc < 16) {
              iWc = 16;
          }

#ifdef __WXMSW__
          pItem = new wxMenuItem(&menuPopup, 1000 + i, caption);
          bModified = caption.EndsWith(wxT("*"));
          if (i == 0) {
              fontT = pItem->GetFont();
          }
          ...
          menuPopup.Append(pItem);
#else
          pItem = new wxMenuItem(&menuPopup, 1000 + i, caption, caption, wxITEM_CHECK);
          if (i == 0) {
              fontT = wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT);
            }
          menuPopup.Append(pItem)->Check(i == (size_t)active_idx);
#endif
          wnd->GetTextExtent(caption, &iX, &iY, NULL, NULL, &fontT);
          if (iX > iXm) {
              iXm = iX;
          }
          //
    ...
    pt.y = cli_rect.y + cli_rect.height;
     pt.x = cli_rect.x + cli_rect.width - (2 + iWc + iXm + iWc + 2);
...
void wxAuiTabContainer::Render(wxDC* raw_dc, wxWindow* wnd)
{
    ...
    int visible_width = 0;
     ...
     arXext.Clear();
           ...
           bool bInsertingPage = (page_count > 1) && IsInsertingPage();
         int iWTW = (int)button_count*16;
      ...
      for (i = 0; i < page_count; ++i)
    {
        ...
        wxSize size = m_art->GetTabSize(dc,
          ...
          arXext.Add(x_extent);
           ...
           bool bOffset = total_width > m_rect.GetWidth();

        // [:COMET:] 20170918 makevisible
     if (bInsertingPage && (iWTW > m_rect.GetWidth())) {
        iWTW = (int)button_count*16;
         for (i = page_count - 1; i >= 0; i--) {
            iWTW += arSizeX[i]+8;
                if (iWTW > m_rect.GetWidth()) {
                    SetTabOffset(((i+1) < (page_count - 1)) ? (i+1) : (page_count - 1));
                    bOffset = true;
                    break;
                }
            }
    ...
    for (i = 0; i < page_count; ++i)
    {
        ...
        {
            if (i+1 < page_count)
                visible_width += arXext[i];
                 else
                visible_width += arSizeX[i];
        }
    }


    if (bOffset || m_tab_offset != 0)
    {
    ...
    for (i = 0; i < button_count; ++i)
    {
        ...
        if (button.id == wxAUI_BUTTON_RIGHT)
        {
          ...
          // [:COMET:] 20170917 disable windowlist button if less than two pages
        if (button.id == wxAUI_BUTTON_WINDOWLIST)
        {
...
// ButtonHitTest() tests if a button was hit. The function returns
// true if a button was hit, otherwise false
bool wxAuiTabContainer::ButtonHitTestLeft(int x, int y,
                                      wxAuiTabContainerButton** hit) const
{
    if (!m_rect.Contains(x,y))
        return false;
    ...
    {
        wxAuiTabContainerButton& button = m_buttons.Item(i);
        if (button.rect.Contains(x,y) && (x < (button.rect.x + 20)))
        {
            if (hit)
                *hit = &button;
            return true;
...
void wxAuiTabCtrl::OnLeftDown(wxMouseEvent& evt)
{
    ...
    if (TabHitTest(evt.m_x, evt.m_y, &wnd))
    {
        ...
        {
              // [:COMET:] 20170920 if close button hit, do not change page if not yet selected
              int old_selection = GetActivePage();
              if ((old_selection == new_selection) || !m_hover_button) {
                  wxAuiNotebookEvent e(wxEVT_COMMAND_AUINOTEBOOK_PAGE_CHANGING, m_windowId);
                  e.SetSelection(new_selection);
                  e.SetOldSelection(old_selection);
...
void wxAuiNotebook::MakeSelectionVisible(void)
{
    if (m_tabs.IsInsertingPage()) {
        return;
    }

    int iPageCount = (int) (GetPageCount());
    int iSel = GetSelection();
    if ((iSel < 0) || (iSel >= iPageCount)) {
        return;
    }
    wxAuiNotebookPage& pageInfo = m_tabs.GetPage(iSel);
    wxAuiTabCtrl* pCtrl;
    int iCtrl;
    if (FindTab(pageInfo.window, &pCtrl, &iCtrl)) {
        wxClientDC dc(pCtrl);
        if (!pCtrl->IsTabVisible(iCtrl, pCtrl->GetTabOffset(), & dc, pCtrl)) {
            for (int i = 0; i < iPageCount; i++) {
                if (pCtrl->IsTabVisible(iCtrl, i, & dc, pCtrl)) {
                    pCtrl->SetTabOffset(i);
                    pCtrl->Refresh();
                    return;
                }
            }
...
void wxAuiTabCtrl::OnButton(wxAuiNotebookEvent& event)
{
    ...
    {
         // [:COMET:] filelist handled externally
        wxAuiNotebookEvent e(wxEVT_COMMAND_AUINOTEBOOK_FILELIST, m_windowId);
        e.SetEventObject(this);
        GetEventHandler()->ProcessEvent(e);
    }
     else
...
BEGIN_EVENT_TABLE(wxAuiNotebook, wxControl)
    ...
    EVT_COMMAND_RANGE(wxAuiBaseTabCtrlId, wxAuiBaseTabCtrlId+500,
     ...
     EVT_COMMAND_RANGE(wxAuiBaseTabCtrlId, wxAuiBaseTabCtrlId+500,
                      wxEVT_COMMAND_AUINOTEBOOK_FILELIST,
                      wxAuiNotebook::OnTabFileList)
     EVT_COMMAND_RANGE(wxAuiBaseTabCtrlId, wxAuiBaseTabCtrlId+500,
                      wxEVT_COMMAND_AUINOTEBOOK_MAINFILELIST,
                      wxAuiNotebook::OnTabMainFileList)
   EVT_COMMAND_RANGE(wxAuiBaseTabCtrlId, wxAuiBaseTabCtrlId+500,
...
bool wxAuiNotebook::AddPage(wxWindow* page,
                      ...
                      bool bFocus/* = true*/)
{
    return InsertPage(GetPageCount(), page, caption, select, bitmap, bFocus);
}
                      ...
                      bool bFocus/* = true*/)
{
    ...
    wxAuiTabCtrl* active_tabctrl = GetActiveTabCtrl();
      ...
      active_tabctrl->SetInsertingPage(true);
    ...
    active_tabctrl->SetInsertingPage(false);
...
bool wxAuiNotebook::RemovePage(size_t page_idx)
{
    ...
    // set new active pane
    m_curpage = -1;
    if (new_active && !m_isBeingDeleted && !IsDeletingAll())
    {
...
size_t wxAuiNotebook::SetSelection(size_t new_page, bool bFocus/* = true*/)
{
    const size_t ret_page = (size_t) m_curpage;
    ...
    {
        // [:COMET:]:150319:MOD: setfocus on window, not on tabctrl
         // [:COMET:]:20170627 add bFocus
         if (bFocus) {
             wxAuiTabCtrl* ctrl;
             int ctrl_idx;
             if (FindTab(wnd, &ctrl, &ctrl_idx)) {
     ...
     wxAuiNotebookEvent evt(wxEVT_COMMAND_AUINOTEBOOK_PAGE_CHANGED, m_windowId);
    evt.SetSelection(new_page);
    evt.SetOldSelection(ret_page);
    evt.SetEventObject(this);
...
void wxAuiNotebook::OnTabFileList(wxCommandEvent& WXUNUSED(evt))
{
            wxAuiNotebookEvent e(wxEVT_COMMAND_AUINOTEBOOK_MAINFILELIST, m_windowId);
        e.SetEventObject(this);
        GetEventHandler()->ProcessEvent(e);
...
void wxAuiNotebook::OnTabMainFileList(wxCommandEvent& WXUNUSED(evt))
{
    // nothing
}
...
// [:COMET:]:170915: move page
void wxAuiNotebook::DoMovePage(int src_idx, int dest_idx)
{
    ...
    if (src_tab) {
        // [:COMET:]:170915: #15709 Page indexing bugs in wxAuiNotebook when pages are moved
    ...
    if (dest_tabs == src_tabs)
    {
        ...
        if (dest_tabs->TabHitTest(pt.x, pt.y, &dest_location_tab))
        {
            ...
            dest_tabs->MovePage(src_tab, dest_idx);
                ...
                // [:COMET:]:170915: #15709 Page indexing bugs in wxAuiNotebook when pages are moved
                int src_page_idx = GetPageIndex(src_tab);
                int dst_page_idx = GetPageIndex(dest_location_tab);
                m_tabs.MovePage(m_tabs.GetPage(src_page_idx).window, dst_page_idx);
                dest_tabs->SetActivePage((size_t)dest_idx);
                SetSelection(dst_page_idx);
