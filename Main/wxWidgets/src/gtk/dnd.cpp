...
static wxDragResult GTKFigureOutSuggestedAction(wxDropTarget *drop_target)
{
	if (!drop_target->m_dragContext)
		return wxDragError;
	...
	const GdkDragAction actions = gdk_drag_context_get_actions(drop_target->m_dragContext);
	if (drop_target->GetDefaultAction() == wxDragNone) {
		...
		{
			suggested_action = ConvertFromGTK(gdk_drag_context_get_suggested_action(drop_target->m_dragContext));
		}
	}
                                    ...
                                    wxDropTarget *drop_target )
{
    ...
    if (drop_target->GetMatchingPair() == (GdkAtom) 0)
    {
        drop_target->SetDragContext( NULL );
        return FALSE;
    }

	 wxDragResult suggested_action = GTKFigureOutSuggestedAction(drop_target);
...
extern "C" {
static void source_drag_failed( GtkWidget          *WXUNUSED(widget),
                                ...
                                wxDropSource       *drop_source )
{
    ...
    // printf( "Drag source: drag_failed.\n" );
...
void wxDropSource::GTKConnectDragSignals()
{
    ...
    g_signal_connect (m_widget, "drag-data-get",
                      G_CALLBACK (source_drag_data_get), this);
    g_signal_connect (m_widget, "drag-failed",
                      G_CALLBACK (source_drag_failed), this);
    g_signal_connect (m_widget, "drag-end",
