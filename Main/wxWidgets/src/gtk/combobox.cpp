...
// [:COMET:]:20170309: added custom popup menu

extern "C" {

static void
    combo_reset_entry(GtkEntry *entry)
{
    wxComboBox *combo = (wxComboBox*) g_object_get_data(G_OBJECT(entry), "wxComboBox_Ptr");
    if (NULL == combo) {
        // should never happen
        return;
    }
    wxCommandEvent event(wxEVT_COMMAND_MENU_SELECTED, wxID_REPLACE_ALL);
    event.SetEventObject(combo);
    combo->GetEventHandler()->ProcessEvent(event);
}
    ...
    gpointer user_data)
{
    ...
    if (false == combo->isResetMenuAdded()) {
        return;
    }
    ...
    g_signal_connect_swapped(item, "activate",
        G_CALLBACK(combo_reset_entry), entry);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
}
}
                         ...
                         const wxString& name )
{
    ...
    if (!gtk_check_version(2,4,0))
    {
        ...
        g_signal_connect_after (m_widget, "changed",
          ...
          g_signal_connect(entry, "populate-popup",
              G_CALLBACK(entry_populate_popup), this);
...
#endif
    {
     ...
     // [:COMET:]:20170309: added custom popup menu
     g_object_set_data(G_OBJECT(entry), "wxComboBox_Ptr", this);
...
{
    // [:COMET:]:20150220: Under GTK, on Linux the last character is not selected
