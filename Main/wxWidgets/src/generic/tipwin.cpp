...
static const wxCoord TEXT_MARGIN_X = 4;
static const wxCoord TEXT_MARGIN_Y = 4;
...
bool wxTipWindow::COLORINITIALIZED = false;
wxColour wxTipWindow::COLORTEXT;
wxColour wxTipWindow::COLORBACK;
wxColour wxTipWindow::COLORBORDER;

// [:COMET:]:151221: add wxFont parameter
...
#endif
{
    ...
    // [:COMET:]:151221: colors
    if (pColorText && pColorBack) {
        // set colours
         wxTipWindow::COLORTEXT = *pColorText;
        wxTipWindow::COLORBACK = *pColorBack;
        wxTipWindow::COLORBORDER = *pColorText;
    }
    else {
         if (wxTipWindow::COLORINITIALIZED == false) {
             wxTipWindow::COLORTEXT = wxSystemSettings::GetColour(wxSYS_COLOUR_INFOTEXT);
             wxTipWindow::COLORBACK = wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOW);
             wxTipWindow::COLORBORDER = wxSystemSettings::GetColour(wxSYS_COLOUR_ACTIVEBORDER);
             wxTipWindow::COLORINITIALIZED = true;
         }
    }
    // set colours
     SetForegroundColour(wxTipWindow::COLORTEXT);
     SetBackgroundColour(wxTipWindow::COLORBACK);

    // set size, position and show it
    m_view = new wxTipWindowView(this);

     wxPoint ptO = ::wxGetMousePosition();

     // [:COMET:]:151221: add wxFont parameter
     if (pFont != NULL) {
         this->SetFont(*pFont);
         m_view->SetFont(*pFont);
         int ix, iy;
         m_view->GetTextExtent(text, &ix, &iy);
         maxLength = ix + 12;
         int iDt = wxMin((ix >> 2) + 4, iy + 8);
         wxRect rcT(ptO.x - iDt, ptO.y - iDt, ix + (iDt << 1), iy + (iDt << 1));
         this->SetBoundingRect(rcT);
     }
     else {
         ...
         //     though
         ptO.y += wxSystemSettings::GetMetric(wxSYS_CURSOR_Y) / 2;
     }
...
void wxTipWindowView::OnPaint(wxPaintEvent& WXUNUSED(event))
{
    ...
    // first filll the background
     dc.SetBrush(wxBrush(wxTipWindow::COLORBACK, wxSOLID));
     dc.SetPen(wxPen(wxTipWindow::COLORBORDER, 1, wxSOLID));
    dc.DrawRectangle(rect);

    // and then draw the text line by line
     dc.SetTextBackground(wxTipWindow::COLORBACK);
     dc.SetTextForeground(wxTipWindow::COLORTEXT);
    dc.SetFont(GetFont());
