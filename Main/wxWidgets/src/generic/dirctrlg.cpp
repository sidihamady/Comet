...
WXDLLEXPORT size_t wxGetAvailableDrives(wxArrayString &paths, wxArrayString &names, wxArrayInt &icon_ids)
{
                              ...
                              const wxString& name)
{
    ...
    if ((style & wxDIRCTRL_3D_INTERNAL) == 0)
     ...
     // [:COMET:]:150807: put filter control on the top
    ...
    m_treeCtrl = CreateTreeCtrl(this, wxID_TREECTRL,
     ...
     // show files with no extension (e.g. Makefile)
     m_strNoExtName = wxEmptyString;
...
void wxGenericDirCtrl::OnEndEditItem(wxTreeEvent &event)
{
    ...
    if ((event.GetLabel().empty()) ||
        ...
        (event.GetLabel().Find(wxT('|')) != wxNOT_FOUND))
    {
         // [:COMET:]:150804: remove wxMessageBox
...
void wxGenericDirCtrl::CollapseDir(wxTreeItemId parentId)
{
    ...
    wxDirItemData *data = (wxDirItemData *) m_treeCtrl->GetItemData(parentId);
     // [:COMET:]:20151026: (data == NULL) test added to avoid crash
     if ((data == NULL) || (data->m_isExpanded == false))
        return;

    data->m_isExpanded = false;

     // [:COMET:]:20151026: Freeze --> Update --> Thaw, to avoid flickering
     wxWindowUpdateLocker flickerFree(m_treeCtrl);
...
void wxGenericDirCtrl::ExpandDir(wxTreeItemId parentId)
{
    wxDirItemData *data = (wxDirItemData *) m_treeCtrl->GetItemData(parentId);
     ...
     // [:COMET:]:20151026: (data == NULL) test added to avoid crash
     if ((data == NULL) || (data->m_isExpanded))
         return;
    ...
    if (d.IsOpened())
    {
         // avoid duplicating files in the explorer (as seen for Makefile)
         wxString eachFilenamePrev = wxEmptyString;
        ...
        if (d.GetFirst(& eachFilename, wxEmptyString, style))
        {
            do
            {
                if ((eachFilenamePrev.IsSameAs(eachFilename) == false) && (eachFilename != wxT(".")) && (eachFilename != wxT("..")))
                {
                         dirs.Add(eachFilename);
                         eachFilenamePrev = eachFilename;
                }
            }
    ...
    if ((GetWindowStyle() & wxDIRCTRL_DIR_ONLY) == 0)
    {
        ...
        if (d.IsOpened())
        {
            ...
            // Process each filter (ex: "JPEG Files (*.jpg;*.jpeg)|*.jpg;*.jpeg")
                ...
                // show files with no extension (e.g. Makefile)
                if (m_strNoExtName.IsEmpty() == false) {
                    if (d.GetFirst(&eachFilename, m_strNoExtName, style)) {
                        do {
                            if ((eachFilenamePrev.IsSameAs(eachFilename) == false) && (eachFilename != wxT(".")) && (eachFilename != wxT(".."))) {
    ...
    if ((GetWindowStyle() & wxDIRCTRL_DIR_ONLY) == 0)
    {
        ...
        for (i = 0; i < filenames.Count(); i++)
        {
            eachFilename = filenames[i];
                ...
                // avoid duplicating files in the explorer (as seen for Makefile)
                if (eachFilename.IsSameAs(eachFilenamePrev)) {
                    continue;
                }
            ...
            wxDirItemData *dir_item = new wxDirItemData(path,eachFilename,false);
                ...
                if (eachFilename.IsSameAs(wxT("Makefile")) || eachFilename.IsSameAs(wxT("makefile"))
                    || eachFilename.StartsWith(wxT("Makefile.")) || eachFilename.StartsWith(wxT("makefile."))) {
                    // Makefile icon in explorer (Makefile, Makefile.*, makefile, makefile.*)
                    image_id = wxFileIconsTable::makefile;
                }
                else {
                    wxFileName fname = eachFilename;
                    wxString strExt = fname.GetExt();
                    if (strExt.IsEmpty() == false) {
                        if (strExt.IsSameAs(wxT("lua"))) {
                            image_id = wxFileIconsTable::luafile;
                        }
                        else if (strExt.IsSameAs(wxT("c"))) {
                            image_id = wxFileIconsTable::cfile;
                        }
                        else if (strExt.IsSameAs(wxT("h")) || strExt.IsSameAs(wxT("hpp")) || strExt.IsSameAs(wxT("hxx")) || strExt.IsSameAs(wxT("hh"))) {
                            image_id = wxFileIconsTable::hfile;
                        }
                        else if (strExt.IsSameAs(wxT("cpp")) || strExt.IsSameAs(wxT("cxx")) || strExt.IsSameAs(wxT("cc")) || strExt.IsSameAs(wxT("C"))) {
                            image_id = wxFileIconsTable::cppfile;
                        }
                        else if (strExt.IsSameAs(wxT("solis"))) {
                            image_id = wxFileIconsTable::infile;
                        }
                        else if (strExt.IsSameAs(wxT("py"))) {
                            image_id = wxFileIconsTable::pyfile;
                        }
                        else if (strExt.IsSameAs(wxT("sh")) || strExt.IsSameAs(wxT("csh")) || strExt.IsSameAs(wxT("bat"))) {
                            image_id = wxFileIconsTable::shfile;
                        }
                        else if (strExt.IsSameAs(wxT("txt")) || strExt.IsSameAs(wxT("conf"))) {
                            image_id = wxFileIconsTable::txtfile;
                        }
                        else if (strExt.IsSameAs(wxT("f")) || strExt.IsSameAs(wxT("for")) || strExt.IsSameAs(wxT("f77")) || strExt.IsSameAs(wxT("f90")) || strExt.IsSameAs(wxT("f95")) || strExt.IsSameAs(wxT("f03")) || strExt.IsSameAs(wxT("f08")) || strExt.IsSameAs(wxT("f15"))) {
                            image_id = wxFileIconsTable::forfile;
                        }
                        else if (strExt.IsSameAs(wxT("m")) || strExt.IsSameAs(wxT("mat"))) {
                            image_id = wxFileIconsTable::mfile;
                        }
                        else if (strExt.IsSameAs(wxT("mak"))) {
                            image_id = wxFileIconsTable::makefile;
                        }
                    }
                }
                // [:COMET:]:150110:MOD: do not get the file association icon
            ...
            (void) AppendItem( parentId, eachFilename, image_id, -1, dir_item);
                ...
                // avoid duplicating 'Makefile' in the explorer
                eachFilenamePrev = eachFilename;
        }
    }
}
...
{
    // [:COMET:]:151026:MOD: expanded tree and restore state
    SaveTreeState();
...
// [:COMET:]:151026:MOD: save the tree state
void wxGenericDirCtrl::SaveTreeItemState()
{
    wxTreeItemId treeId = (wxTreeItemId) (m_treeCtrl->GetSelection());
    if (treeId.IsOk()) {
        wxDirItemData *pData = (wxDirItemData*) (m_treeCtrl->GetItemData(treeId));
        if (pData != NULL) {
            m_expandedPath = pData->m_path;
        }
    }
}
void wxGenericDirCtrl::SaveTreeState()
{
...
// [:COMET:]:151026:MOD: restore tree state
void wxGenericDirCtrl::RestoreTreeState()
{
    if (m_expandedPath.IsEmpty() == false) {
        // file parent dir is selected at this stage
        this->ExpandPath(m_expandedPath);
    }
...
bool wxGenericDirCtrl::ExtractWildcard(const wxString& filterStr, int n, wxString& filter, wxString& description)
{
    ...
    if (count > 0 && n < count)
    {
        ...
        description = descriptions[n];
          ...
          // show files with no extension (e.g. Makefile)
          if (filter.IsEmpty() && (description.IsEmpty() == false)) {
              bool bOK = true;
              const wxString strForbiddenChars = wxFileName::GetForbiddenChars();
              for (size_t ii = 0; ii < description.Len(); ii++) {
                  if (strForbiddenChars.Contains(description[ii]) || (description[ii] == wxT('!')) || (description[ii] == wxT(' '))) {
                      bOK = false;
                      break;
                  }
              }
              if (bOK) {
                  m_strNoExtName = description;
              }
          }
          //
     ...
     if (m_treeCtrl)
    {
        wxSize filterSz ;
          ...
          if (m_filterListCtrl) {
              ...
              sz.y -= (filterSz.y + verticalSpacing + verticalSpacingZ);
              m_filterListCtrl->SetSize(verticalSpacing, verticalSpacing, sz.x - (verticalSpacing << 1), filterSz.y);
          }
          m_treeCtrl->SetSize(0, filterSz.y + verticalSpacing + verticalSpacingZ, sz.x, sz.y);
          if (m_filterListCtrl) {
...
void wxFileIconsTable::Create()
{
    ...
    m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_NORMAL_FILE,
          ...
          // [:COMET:]:150228:MOD: add file types
          m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_LUA_FILE,
              wxART_CMN_DIALOG,
              wxSize(16, 16)));
          m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_H_FILE,
              wxART_CMN_DIALOG,
              wxSize(16, 16)));
          m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_C_FILE,
              wxART_CMN_DIALOG,
              wxSize(16, 16)));
          m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_CPP_FILE,
              wxART_CMN_DIALOG,
              wxSize(16, 16)));
          m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_IN_FILE,
              wxART_CMN_DIALOG,
              wxSize(16, 16)));
          m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_PY_FILE,
              wxART_CMN_DIALOG,
              wxSize(16, 16)));
          m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_SH_FILE,
              wxART_CMN_DIALOG,
              wxSize(16, 16)));
          m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_TXT_FILE,
              wxART_CMN_DIALOG,
              wxSize(16, 16)));
          m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_FOR_FILE,
              wxART_CMN_DIALOG,
              wxSize(16, 16)));
          m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_M_FILE,
              wxART_CMN_DIALOG,
              wxSize(16, 16)));
          m_smallImageList->Add(wxArtProvider::GetBitmap(wxART_MAK_FILE,
              wxART_CMN_DIALOG,
              wxSize(16, 16)));
        //
