...
//
// Created:     13-Jan-2000
// RCS-ID:      $Id: stc.cpp 66923 2011-02-16 22:37:48Z JS $
...
#endif
     // >> [:COMET:]:130531:
     // added ~wxVSCROLL & ~wxHSCROLL
    if (!wxControl::Create(parent, id, pos, size,
                           (style | wxWANTS_CHARS | wxCLIP_CHILDREN) & ~wxVSCROLL & ~wxHSCROLL,
...
int wxStyledTextCtrl::MarkerAdd(int line, int markerNumber) {
    int iRet = (int) SendMsg(2043, line, markerNumber);
...
void wxStyledTextCtrl::setMarkersColors(unsigned long lBackModified, unsigned long lBackSaved, unsigned long lBackFind, long unsigned lBackError)
{
    if (m_swx) {
        m_swx->setMarkersColors(lBackModified, lBackSaved, lBackFind, lBackError);
    }
}
void wxStyledTextCtrl::getMarkersColors(unsigned long *lBackModified, unsigned long *lBackSaved, unsigned long *lBackFind, unsigned long *lBackError)
{
    if (m_swx) {
        m_swx->getMarkersColors(lBackModified, lBackSaved, lBackFind, lBackError);
    }
}
//
...
void wxStyledTextCtrl::AutoCompShow(int lenEntered, const wxString& itemList, int iSel) {
    //SendMsg(2100, lenEntered, (wxIntPtr)(const char*)wx2stc(itemList));
    m_swx->DoAutoCompeteShow(lenEntered, (const char*) wx2stc(itemList), iSel);
}

// [:COMET:]:150122: Update list (to avoid flickering)
void wxStyledTextCtrl::AutoCompUpdate(int lenEntered, const wxString& itemList, int iSel, bool bComplete) {
    //SendMsg(2100, lenEntered, (wxIntPtr)(const char*)wx2stc(itemList));
    m_swx->DoAutoCompleteUpdate(lenEntered, (const char*) wx2stc(itemList), iSel, bComplete);
}
...
// Select the item in the auto-completion list that starts with a string.
// [:COMET:]:20150122:
void wxStyledTextCtrl::AutoCompSelectRaw(const char* text) {
    SendMsg(2108, 0, (wxIntPtr) text);
}
//

// Select the item in the auto-completion list that has index idx.
// [:COMET:]:20150122:
void wxStyledTextCtrl::AutoCompSelectIndex(int idx) {
    SendMsg(SCI_AUTOCSELECTINDEX, 0, (wxIntPtr) idx);
}
//
...
// >> [:COMET:]:150310: add findMinPos and findMaxPos
int wxStyledTextCtrl::FindText(int minPos, int maxPos,
               const wxString& text,
                    int flags,
                    int *findMinPos/* = NULL*/, int *findMaxPos/* = NULL*/) {
            TextToFind  ft;
            ...
            int iPos = (int) SendMsg(2150, flags, (wxIntPtr)&ft);
                if (findMinPos) {
                    *findMinPos = ft.chrgText.cpMin;
                }
                if (findMaxPos) {
                    *findMaxPos = ft.chrgText.cpMax;
                }
                return iPos;
}
...
// >> [:COMET:]:140314:
void wxStyledTextCtrl::SetExtraAscent(int iAscent) {
    SendMsg(SCI_SETEXTRAASCENT, iAscent, 0);
}
void wxStyledTextCtrl::SetExtraDescent(int iDescent) {
    SendMsg(SCI_SETEXTRADESCENT, iDescent, 0);
}
// <<
...
// [:COMET:]:20150123:
void wxStyledTextCtrl::setModeScriptConsole(bool bModeScriptConsole)
{
    m_swx->setModeScriptConsole(bModeScriptConsole);
}
bool wxStyledTextCtrl::isModeScriptConsole(void)
{
    return m_swx->isModeScriptConsole();
}

//

// [:COMET:]:160106:MOD: added alwaysShowCaret
void wxStyledTextCtrl::setAlwaysShowCaret(bool alwaysShowCaret)
{
    m_swx->setAlwaysShowCaret(alwaysShowCaret);
}
//
