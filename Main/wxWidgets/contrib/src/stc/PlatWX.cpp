...
void SurfaceImpl::DrawTextClipped(PRectangle rc, Font &font, int ybase,
    ...
    hdc->SetClippingRegion(wxRectFromPRectangle(rc));
     wxSize sizeT = hdc->GetTextExtent(wxT(">"));

     // [:COMET:]:20150130:
     int centreX = (rc.right + rc.left) >> 1;
     int centreY = (rc.bottom + rc.top) >> 1;
     ybase = centreY - (sizeT.GetHeight() >> 1);
     rc.left = centreX - (sizeT.GetWidth() >> 1);
     //

    // see comments above
     hdc->DrawText(stc2wx(s, len), rc.left, ybase);
...
void Window::InvalidateAll(bool eraseBackground/* = false*/)
{
    GETWIN(id)->Refresh(eraseBackground);
}
...
public:
    ...
    {
         // [:COMET:]:20150122: Change border color
         SetBackgroundColour(wxColour(127,169,205)/**wxBLACK*/);  // for our simple border
         //
