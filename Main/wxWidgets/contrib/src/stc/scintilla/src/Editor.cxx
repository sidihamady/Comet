...
// >> [:COMET:]:140328:
#define STC_COLOR(r,g,b)                       ((unsigned long)(((unsigned char)(r) | ((unsigned short)((unsigned char)(g)) << 8)) | (((unsigned short)(unsigned char)(b)) << 16)))
#define STC_COLORB(rgb)                        ((unsigned char)((((unsigned short)(rgb >> 16))) & 0xFF))
#define STC_COLORG(rgb)                        ((unsigned char)((((unsigned short)(rgb >> 8))) & 0xFF))
#define STC_COLORR(rgb)                        ((unsigned char)(((unsigned short)(rgb)) & 0xFF))
// <<
...
Editor::Editor() {
    ...
    // [:COMET:]:160106:MOD: added alwaysShowCaret
    ...
    // [:COMET:]:20150123:
    modeScriptConsole = false;
    //

    // [:COMET:]:20151117: add markers colors
    clrBackModified = ColourAllocated(0x00B1E6);
    clrBackSaved = ColourAllocated(0x009700);
    clrBackFind = ColourAllocated(0xC24230);
    clrBackError = ColourAllocated(0x0000FF);
    //
...
void Editor::PaintSelMargin(Surface *surfWindow, PRectangle &rc) {
    ...
    // >> [:COMET:]:140328: Create a contrast between margin colors
    long lBack = vs.styles[STYLE_LINENUMBER].back.allocated.AsLong();
    int iCodeID = 1;
    int iStatusID = 3;
    // <<
        ...
        if (vs.ms[margin].width > 0) {
            ...
            if (vs.ms[margin].style != SC_MARGIN_NUMBER) {
                ...
                if (vs.ms[margin].mask & SC_MASK_FOLDERS)
                    ...
                    // >> [:COMET:]:140328:
                    if (margin == iStatusID) {
                        /*long iB = vs.styles[STYLE_DEFAULT].back.allocated.AsLong();
                        long iF = vs.styles[STYLE_DEFAULT].fore.allocated.AsLong();
                        lBack = normalizeColor(iB, (iB > iF) ? -16L : 16L);*/
                        lBack = vs.styles[STYLE_DEFAULT].back.allocated.AsLong();
                    }
                    else if (margin == iCodeID) {
                        long iB = vs.styles[STYLE_LINENUMBER].back.allocated.AsLong();
                        long iF = vs.styles[STYLE_LINENUMBER].fore.allocated.AsLong();
                        lBack = normalizeColor(iB, (iB > iF) ? -16L : 16L);
                    }
                    else {
                        lBack = vs.styles[STYLE_LINENUMBER].back.allocated.AsLong();
                    }
                    ColourAllocated clrBack(lBack);
                    surface->FillRectangle(rcSelMargin, clrBack);
                    // <<
                }
            } else {
                surface->FillRectangle(rcSelMargin, vs.styles[STYLE_LINENUMBER].back.allocated);
            ...
            while ((visibleLine < cs.LinesDisplayed()) && yposScreen < rcMargin.bottom) {
                ...
                if (marks) {
                    for (int markBit = 0; (markBit < 32) && marks; markBit++) {
                        if (marks & 1) {
                            // >> [:COMET:]:150130: DrawChar added
                            if (vs.markers[markBit].markType >= SC_MARK_CHARACTER) {
                                if (vs.markers[markBit].markType == (SC_MARK_CHARACTER + '!')) {
                                    vs.markers[markBit].DrawBack(surface, rcMarker, clrBackError);
                                }
                                else if (vs.markers[markBit].markType == (SC_MARK_CHARACTER + 'f')) {
                                    vs.markers[markBit].DrawBack(surface, rcMarker, clrBackFind);
                                }
                                else if (vs.markers[markBit].markType == (SC_MARK_CHARACTER + 'm')) {
                                    vs.markers[markBit].DrawBack(surface, rcMarker, clrBackModified);
                                }
                                else if (vs.markers[markBit].markType == (SC_MARK_CHARACTER + 's')) {
                                    vs.markers[markBit].DrawBack(surface, rcMarker, clrBackSaved);
                                }
                                else {
                                    long iB = vs.styles[STYLE_DEFAULT].back.allocated.AsLong();
                                    long iF = vs.styles[STYLE_DEFAULT].fore.allocated.AsLong();
                                    //iB = normalizeColor(iB, (iB > iF) ? -16L : 16L);
                                    if (vs.markers[markBit].markType == (SC_MARK_CHARACTER + '>')) {
                                        iF = modifyColor(iF, 1, 128L);
                                    }
                                    else if (vs.markers[markBit].markType == (SC_MARK_CHARACTER + '#')) {
                                        iF = modifyColor(iF, 2, 128L);
                                    }
                                    ColourAllocated clrBack = ColourAllocated(iB);
                                    ColourAllocated clrFore = ColourAllocated(iF);
                                    vs.markers[markBit].DrawChar(surface, rcMarker, vs.styles[STYLE_DEFAULT].font, clrFore, clrBack);
                                }
                            }
                            else {
                                vs.markers[markBit].Draw(surface, rcMarker, vs.styles[STYLE_DEFAULT].font);
                            }
                            break;
                        }
                        marks >>= 1;
...
void DrawTabArrow(Surface *surface, PRectangle rcTab, int ymid) {
    int ydiff = ((rcTab.bottom - rcTab.top) >> 1) - 1;
    int xleft = rcTab.left + 2;
    int xright = rcTab.right - 2;
    if (xright <= xleft) {
        xright = xleft + 4;
    }
    int xmid = xright - ydiff;
    surface->MoveTo(xleft, ymid);
    surface->LineTo(xright, ymid);
    surface->LineTo(xmid, ymid - ydiff);
    surface->MoveTo(xright, ymid);
    surface->LineTo(xmid, ymid + ydiff);
}
...
// Format: OxBBGGRR
long Editor::normalizeColor(long iColor, long iDelta)
{
    long iB = (iColor >> 16L) + iDelta;
    ...
    long iG = ((iColor >> 8L) & 0x00FF) + iDelta;
    ...
    long iR = (iColor & 0x0000FF) + iDelta;
...
long Editor::modifyColor(long iColor, int iComp, long iDelta)
{
    long iR = (iColor & 0x0000FF);
    long iG = ((iColor >> 8L) & 0x00FF);
    long iB = (iColor >> 16L);
    ...
    if (!modeScriptConsole) {
        if (caret.active && vsDraw.showCaretLineBackground && (vsDraw.caretLineAlpha == SC_ALPHA_NOALPHA) && ll->containsCaret) {
            ...
            if (hasFocus) {
                ...
                long iB = vs.styles[STYLE_DEFAULT].back.allocated.AsLong();
                long iF = vs.styles[STYLE_DEFAULT].fore.allocated.AsLong();
                long lBack = normalizeColor(iB, (iB > iF) ? -16L : 16L);
                ColourAllocated clrBack(lBack);
                background = clrBack;
            }
        }
        ...
        int iLinesCount = (pdoc->LinesTotal() == 0) ? 1 : pdoc->LinesTotal();
        if (line == (iLinesCount - 1)) {
            overrideBackground = true;
            background = vsDraw.caretLineBackground.allocated;
        }
        else {
                ...
                if (ll->chars[i] == '\t') {
                    ...
                    if (vsDraw.viewWhitespace != wsInvisible) {
                        if (!inIndentation || vsDraw.viewWhitespace == wsVisibleAlways) {
                            // [:COMET:]:150305:MOD: TAB style
                            int iTabLen = 14;
                            if (iTabLen > (rcSegment.right - rcSegment.left)) {
                                iTabLen = (rcSegment.right - rcSegment.left);
                            }
                            else if (iTabLen < ((rcSegment.right - rcSegment.left) >> 1)) {
                                iTabLen = (rcSegment.right - rcSegment.left) >> 1;
                            }
                            int iTabHeight = 10;
                            if (iTabHeight > (((rcSegment.bottom - rcSegment.top) >> 1) + 2)) {
                                iTabHeight = ((rcSegment.bottom - rcSegment.top) >> 1) + 2;
                            }
                            else if (iTabHeight < ((rcSegment.bottom - rcSegment.top) >> 2)) {
                                iTabHeight = (rcSegment.bottom - rcSegment.top) >> 2;
                            }
                            PRectangle rcTab(rcSegment.left + 1, rcSegment.top + 4,
                                rcSegment.left + iTabLen, rcSegment.top + 4 + iTabHeight);
                    ...
                    if (controlCharSymbol < 32) {
                        // Draw the character
                        // [:COMET:]:150305:MOD: EOL colors >>
                        if ((ll->chars[i] == '\r') || (ll->chars[i] == '\n')) {
                            ...
                            rcCChar.bottom = rcCChar.top + normalCharHeight + 1;
                            PRectangle rcCCChar = rcCChar;
                            rcCCChar.left--;
                            rcCCChar.right++;
                            rcCCChar.bottom += 3;
                            surface->FillRectangle(rcCCChar, vsDraw.styles[STYLE_LINENUMBER].back.allocated);
                            rcCChar.left++;
                            surface->DrawTextTransparent(rcCChar, ctrlCharsFont,
                                rcSegment.top + vsDraw.maxAscent, ctrlChar, istrlen(ctrlChar),
                                vsDraw.styles[STYLE_LINENUMBER].fore.allocated);
                        }
                        // <<
                    } else {
                        char cc[2] = { static_cast<char>(controlCharSymbol), '\0' };
                            ...
                            if (ll->chars[cpos + startseg] == ' ') {
                                if (vsDraw.viewWhitespace != wsInvisible) {
                                    ...
                                    if (!inIndentation || vsDraw.viewWhitespace == wsVisibleAlways) {
                                        ...
                                        surface->FillRectangle(rcDot, vsDraw.styles[STYLE_LINENUMBER].fore.allocated);
                                    }
                                }
                                if (inIndentation && vsDraw.viewIndentationGuides) {
...
void Editor::RefreshPixMaps(Surface *surfaceWindow) {
    ...
    if (!pixmapIndentGuide->Initialised()) {
        ...
        // [:COMET:]:150308: indent guide colors
        pixmapIndentGuide->FillRectangle(rcIG, vs.styles[STYLE_DEFAULT].back.allocated);
        pixmapIndentGuide->PenColour(vs.styles[STYLE_LINENUMBER].fore.allocated);
        pixmapIndentGuideHighlight->FillRectangle(rcIG, vs.styles[STYLE_DEFAULT].back.allocated);
        pixmapIndentGuideHighlight->PenColour(vs.styles[STYLE_LINENUMBER].fore.allocated);
...
// [:COMET:]:20151117: add markers colors
// Color in 0xGGBBRR format
void Editor::setMarkersColors(unsigned long lBackModified, unsigned long lBackSaved, unsigned long lBackFind, unsigned long lBackError)
{
    clrBackModified = ColourAllocated(STC_COLOR(STC_COLORR(lBackModified), STC_COLORG(lBackModified), STC_COLORB(lBackModified)));
    clrBackSaved = ColourAllocated(STC_COLOR(STC_COLORR(lBackSaved), STC_COLORG(lBackSaved), STC_COLORB(lBackSaved)));
    clrBackFind = ColourAllocated(STC_COLOR(STC_COLORR(lBackFind), STC_COLORG(lBackFind), STC_COLORB(lBackFind)));
    clrBackError = ColourAllocated(STC_COLOR(STC_COLORR(lBackError), STC_COLORG(lBackError), STC_COLORB(lBackError)));
}
void Editor::getMarkersColors(unsigned long *lBackModified, unsigned long *lBackSaved, unsigned long *lBackFind, unsigned long *lBackError)
{
    *lBackModified = clrBackModified.AsLong();
    *lBackSaved = clrBackSaved.AsLong();
    *lBackFind = clrBackFind.AsLong();
    *lBackError = clrBackError.AsLong();
}
//
    ...
    if (rcArea.right > vs.fixedColumnWidth) {
        ...
        while (visibleLine < cs.LinesDisplayed() && yposScreen < rcArea.bottom) {
            ...
            if (ll) {
                ...
                // [:COMET:]:150319:MOD: added hasFocus
                // [:COMET:]:160106:MOD: added alwaysShowCaret
                if ((lineDoc == lineCaret) && (hasFocus || alwaysShowCaret)) {
...
// [:COMET:]:150901: lineNumberPrintSpace removed
...
long Editor::FormatRange(bool draw, RangeToFormat *pfr) {
    ...
    // [:COMET:]:150901: xStart decreased 
    int xStart = (vsPrint.fixedColumnWidth >> 1) + pfr->rc.left + lineNumberWidth;
    int ypos = pfr->rc.top;
...
void Editor::ButtonMove(Point pt) {
    ...
    if (HaveMouseCapture()) {
        ...
        // [:COMET:]:20151215: Fix automatic scrolling when select+click+drag.
        EnsureCaretVisible(false, true, true);
...
void Editor::SetFocusState(bool focusState) {
    ...
    if (hasFocus) {
        ...
        CancelModes();
        // [:COMET:]:150319:MOD: removed DropCaret
        //DropCaret();
    }
}
...
sptr_t Editor::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
    ...
    case SCI_REPLACESEL: {
            ...
            //EnsureCaretVisible();
            // [:COMET:]:170906: select the replacement
            SetSelection(currentPos, currentPos + istrlen(replacement));
        }
        break;
