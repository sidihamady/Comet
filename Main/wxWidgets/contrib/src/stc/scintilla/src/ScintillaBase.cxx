...
void ScintillaBase::DoAutoCompleteUpdate(int lenEntered, const char *list, int iSel, bool bComplete) {
    ...
    // [:COMET:]BUG:20150122: add select index before calling AutoCompleteMoveToCurrentWord. If not, the completion window is cancelled.
    ac.SelectIndex(iSel);
    if (lenEntered != 0) {
...
// [:COMET:]:150122: Show list AND select item
void ScintillaBase::AutoCompleteStart(int lenEntered, const char *list, int iSel) {
...
void ScintillaBase::AutoCompleteMoveToCurrentWord() {
    ...
    //[:COMET:]BUG: if Select called the completion list is cancelled.
...
// [:COMET:]:150122: Show list AND select item
void ScintillaBase::DoAutoCompeteShow(int lenEntered, const char *list, int iSel)
{
    AutoCompleteStart(lenEntered, list, iSel);
    return;
}
...
sptr_t ScintillaBase::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
    ...
    case SCI_AUTOCSHOW:
        listType = 0;
        AutoCompleteStart(wParam, reinterpret_cast<const char *>(lParam), 2);
        break;
    ...
    // [:COMET:]:20150122:
    case SCI_AUTOCSELECTINDEX:
        ac.SelectIndex((int) (lParam));
        break;
    //
    ...
    case SCI_USERLISTSHOW:
        listType = wParam;
        AutoCompleteStart(0, reinterpret_cast<const char *>(lParam), 2);
        break;
