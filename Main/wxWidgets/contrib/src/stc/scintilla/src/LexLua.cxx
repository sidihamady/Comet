...
static void ColouriseLuaDoc(
    ...
    for (; sc.More(); sc.Forward()) {
        ...
        if (sc.state == SCE_LUA_OPERATOR) {
            ...
            // [:COMET:]:160107:MOD: handle operator inside number
            if (IsLuaOperator(static_cast<char>(sc.ch)) && (sc.ch != '.')) {
                sc.SetState(SCE_LUA_OPERATOR);
            }
            else if (!IsANumberChar(sc.ch)) {
                sc.SetState(SCE_LUA_DEFAULT);
            }
        ...
        if (sc.state == SCE_LUA_DEFAULT) {
            if (IsADigit(sc.ch) || (sc.ch == '.' && IsADigit(sc.chNext))) {
                ...
                if (sc.ch == '0' && (toupper(sc.chNext) == 'X' || toupper(sc.chNext) == 'O' || toupper(sc.chNext) == 'B' || toupper(sc.chNext) == 'D')) {
                    sc.Forward(1);
                }
