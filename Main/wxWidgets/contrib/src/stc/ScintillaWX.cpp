...
#if wxUSE_DRAG_AND_DROP
// [:COMET:]:150713: add fix to drag text on Linux
#if defined(__WXGTK__)
    // For recent wxGTKs, DnD won't work if we delay with the timer, so go there direct
    // DoStartDrag disabled: not compatible with drop files
    //DoStartDrag();
#else
...
void ScintillaWX::DoStartDrag() {
    ...
    if (dragText.length()) {
         // [:COMET:]:150713: undo in a single Ctrl-Z
...
void ScintillaWX::DoRedraw(void)
{
    wMain.InvalidateAll(true);
}
...
// [:COMET:]:20150123:
void ScintillaWX::setModeScriptConsole(bool bModeScriptConsole)
{
    this->modeScriptConsole = bModeScriptConsole;
}
bool ScintillaWX::isModeScriptConsole(void)
{
    return this->modeScriptConsole;
}

//

// [:COMET:]:160106:MOD: added alwaysShowCaret
void ScintillaWX::setAlwaysShowCaret(bool alwaysShowCaret)
{
    this->alwaysShowCaret = alwaysShowCaret;
}
//
