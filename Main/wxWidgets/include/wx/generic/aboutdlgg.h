...
public:
    ...
    // ctor which fully initializes the object
	 wxGenericAboutDialog(const wxAboutDialogInfo& info, wxWindow *pParent)
    {
        Init();

		  (void) Create(info, pParent);
    }

    // this method must be called if and only if the default ctor was used
	 bool Create(const wxAboutDialogInfo& info, wxWindow *pParent);
...
WXDLLIMPEXP_ADV void wxGenericAboutBox(const wxAboutDialogInfo& info, wxWindow *pParent);
